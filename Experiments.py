...
from tensorflow.keras.layers import Dense, Conv2D, MaxPooling2D, Flatten, BatchNormalization, Dropout, SpatialDropout2D, LeakyReLU, Reshape, Conv2DTranspose, Input, Embedding, Concatenate
...
def summarize_performance(epoch, g_model, d_model, g_loss, d_loss, images, labels, latent_dim, n_samples=100):
    
    [X_real, X_real_labels], y_real = generate_real_samples(images, labels, n_samples)
    _, acc_real = d_model.evaluate([X_real, X_real_labels], y_real, verbose=0)
    
    [X_fake, X_fake_labels], y_fake = generate_fake_samples(g_model, latent_dim, n_samples)
    _, acc_fake = d_model.evaluate([X_fake, X_fake_labels], y_fake, verbose=0)
       
    #Printing Discriminator Performance
    print('epoch: %03d, acc_real: %.02d%%, acc_fake: %.02d%%, d=%.3f, g=%.3f' % (epoch + 1, round(acc_real,2)*100, round(acc_fake,2)*100, g_loss, d_loss))

    #Get it to create the folder: Models if it does not exist

    #CSV File of Discriminator Performance
    with open(r'C:/Users/Luke Ginn/Documents/Python Scripts/2021-09-07 - cGAN/Models/Discriminator_Evaluation.csv','a', newline='') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=['Epoch', 'Acc_Real', 'Acc_Fake', 'G_Loss', 'D_Loss'])
        writer.writerow({'Epoch': epoch + 1, 'Acc_Real':str(acc_real), 'Acc_Fake':str(acc_fake), 'G_Loss': g_loss, 'D_Loss': d_loss})
           
    #Plot of Discriminator Performance
    save_plot(X_fake, epoch, acc_real, acc_fake)
    
    #Save GAN
    filename = 'C:/Users/Luke Ginn/Documents/Python Scripts/2021-09-07 - cGAN/Models/gan_model_%03d.h5' % (epoch + 1)
    d_model.trainable = False
    gan_model.save(filename)
    
    #Save Generator
    filename = 'C:/Users/Luke Ginn/Documents/Python Scripts/2021-09-07 - cGAN/Models/generator_model_%03d.h5' % (epoch + 1)
    d_model.trainable = True
    g_model.save(filename)
    
    #Save Discriminator
    filename = 'C:/Users/Luke Ginn/Documents/Python Scripts/2021-09-07 - cGAN/Models/discriminator_model_%03d.h5' % (epoch + 1)
    d_model.trainable = True
...
def discriminator(n_classes = 10):
    ...
    #model = load_model('C:/Users/Luke Ginn/Documents/Python Scripts/2021-09-07 - cGAN/Models/discriminator_model_065.h5')
...
def generator(latent_dim, n_classes=10):
    ...
    NeuralNet = SpectralNormalization(Conv2DTranspose(128, (4,4), strides=(1,1)))(NeuralNet)
    NeuralNet = LeakyReLU(alpha=0.2)(NeuralNet)
    NeuralNet = BatchNormalization()(NeuralNet)
    NeuralNet = SpectralNormalization(Conv2DTranspose(128, (4,4), strides=(2,2)))(NeuralNet)
    NeuralNet = LeakyReLU(alpha=0.2)(NeuralNet)
    NeuralNet = BatchNormalization()(NeuralNet)
    NeuralNet = SpectralNormalization(Conv2DTranspose(128, (5,5), strides=(2,2), padding='same'))(NeuralNet)
    NeuralNet = LeakyReLU(alpha=0.2)(NeuralNet)
    NeuralNet = BatchNormalization()(NeuralNet)
    NeuralNet = SpectralNormalization(Conv2D(1, (10,10), activation='sigmoid', padding='same'))(NeuralNet)
    
    model = Model([Latent_Input, Label_Input], NeuralNet)
        
    #model = load_model('C:/Users/Luke Ginn/Documents/Python Scripts/2021-09-07 - cGAN/Models/generator_model_065.h5')
...
def GAN(g_model, d_model):  
    ...
    gen_Latent_Input, gen_Label_Input = g_model.input
    gen_Image_Output = g_model.output
    dis_prediction = d_model([gen_Image_Output, gen_Label_Input])
    model = Model([gen_Latent_Input, gen_Label_Input], dis_prediction)
      
    #model = load_model('C:/Users/Luke Ginn/Documents/Python Scripts/2021-09-07 - cGAN/Models/gan_model_024.h5')
...
def GAN_Train(g_model, d_model, gan_model, images, labels, latent_dim, start_epoch = 0, n_epochs=100, n_batch=256):
    ...
    for i in range(start_epoch, n_epochs):
        for j in range(bat_per_epo):
            ...
            [X_real, X_real_labels], y_real = generate_real_samples(images, labels, half_batch)
            [X_fake, X_fake_labels], y_fake = generate_fake_samples(g_model, latent_dim, half_batch)
            
            X, y = np.vstack((X_real, X_fake)), np.vstack((y_real, y_fake))
            X_labels = np.concatenate((X_real_labels, X_fake_labels))
            
            d_loss, _ = d_model.train_on_batch([X, X_labels], y)
        ...
        #if (i+1) % 10 == 0:
        summarize_performance(i, g_model, d_model, g_loss, d_loss, images, labels, latent_dim)
...
GAN_Train(g_model, d_model, gan_model, train_images, train_labels, latent_dim, start_epoch = 0)
#Turning off the GPU
sess.close()
